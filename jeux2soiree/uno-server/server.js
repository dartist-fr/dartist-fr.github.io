...
const STACKING_AUTO_DRAW_MS = 3500;
...
function send(socket, data) {
    ...
    if (socket.readyState === WebSocket.OPEN) {
        try {
            ...
            console.error("Erreur WebSocket send :", error.message);
        }
    }
}
...
function broadcast(room, data) {
    ...
    const hostIsPlayer = room.players.some(
        player => player.socket === room.host
    );
...
function canPlayWild4(room, player) {
    // Règle personnalisée :
    // le +4 peut être joué à tout moment.
    return true;
}
...
function basePlayable(room, player, card) {
    ...
    if (card.type === "wild4") {
        return canPlayWild4(room, player);
    }
...
function publicState(room, viewerId) {
    ...
    return {
        ...
        canPlayWild4: viewer
            ? canPlayWild4(room, viewer)
            : false,

        isHost: viewer
            ? viewer.socket === room.host
            : true
    };
}
...
function makeRoom(
    ...
    // En mode téléphones uniquement,
    // le créateur est également joueur.
    if (room.mode === "phones") {
        const player = {
            id: makeId(),
...
function startGame(room) {
    ...
    // Distribution une carte à la fois.
    for (
        let round = 0;
        round < room.settings.handSize;
        round++
    ) {
        for (const player of room.players) {
            const card = drawOne(room);
...
function scheduleStackingAutoDraw(
    room,
    targetId
) {
    clearPendingDrawTimer(room);
    ...
    room.pendingDrawTimer =
        setTimeout(() => {
            ...
            addLog(
                room,
                `⚠️ ${target.name} ne répond pas avec un +2 : il pioche automatiquement ${drawn} carte(s) (+${amount}).`
            );
...
// =========================================================
// UNO - APPLICATION PÉNALITÉ IMMÉDIATE
// =========================================================
...
function advanceAfterCard(
    ...
    if (card.type === "skip") {
        ...
        if (actor && blocked) {
            addLog(
                room,
                `⛔ ${blocked.name} est bloqué par ${actor.name}.`
            );
    ...
    if (card.type === "reverse") {
        ...
        if (room.players.length === 2) {
            ...
            if (actor && blocked) {
                addLog(
                    room,
                    `↔ ${actor.name} inverse le sens et bloque ${blocked.name}.`
                );
            ...
            if (actor) {
                ...
                addLog(
                    room,
                    `↔ ${actor.name} inverse le sens de jeu.`
                );
    ...
    if (card.type === "draw2") {
        ...
        if (room.settings.stacking) {
            ...
            if (actor && target) {
                addLog(
                    room,
                    `➕ ${target.name} doit prendre +${penaltyAmount}, mais peut empiler un +2.`
                );
        ...
        if (actor && target) {
            addLog(
                room,
                `➕ ${target.name} se prend +${penaltyAmount} de la part de ${actor.name}.`
            );
...
function playCard(
    ...
    // En empilement, seul un +2
    // peut répondre à une pénalité.
    if (
        room.settings.stacking &&
        room.pendingDraw > 0 &&
        card.type !== "draw2"
        ...
        send(player.socket, {
            ...
            message:
                `Tu dois jouer un +2 pour empiler ou attendre la pioche automatique de ${room.pendingDraw} carte(s).`
        });
    ...
    // Si le joueur joue le +2
    // pendant l'empilement,
    // annuler le timer avant de continuer.
    if (
        card.type === "draw2" &&
        room.settings.stacking &&
        room.pendingDraw > 0
...
function drawPenalty(
    ...
    if (
        ...
        addLog(
            room,
            `⚠️ ${player.name} pioche ${drawn} carte(s) de pénalité (+${amount}).`
        );
...
function battlePlaceShip(
    ...
    // avant de vérifier le placement.
    for (
        const rowCells of
        player.board
    ) {
    ...
    if (!placement.ok) {
        // Si le navire existait,
        // on le remettra lors d'un
        // prochain placement.
        return placement;
    }
...
function battleShoot(
    ...
    if (result === "miss") {
        addLog(
            room,
            `💦 ${player.name} tire en ${String.fromCharCode(65 + col)}${row + 1} : À L'EAU !`
        );
    } else if (
        result === "hit"
    ) {
        addLog(
            room,
            `💥 ${player.name} touche un navire en ${String.fromCharCode(65 + col)}${row + 1} !`
        );
    } else {
        addLog(
            room,
...
const heartbeatInterval =
    setInterval(() => {
        for (
            ...
            // On considère la connexion
            // morte si elle n'a pas répondu
            // au ping précédent.
            if (
                socket.isAlive ===
                false
